...
class Project {
  ...
  startInterval() {
    ...
    this._startedAt = Date.now(); // now
    this._currentInterval = setInterval(() => {
      // calulate the number of seconds
      let secondsDiff = (Date.now() - this._startedAt) / 1000;
      secondsDiff *= this.timeScaler;
      this.totalTimeInSeconds += secondsDiff;
      this.totalTimeInSeconds = Math.round(this.totalTimeInSeconds);
      updateUI(true);
